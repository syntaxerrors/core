<?php namespace Syntax\Core\Ardent;
...
use Symfony\Component\Translation\FileLoader;
	...
	protected function handleRelationalArray($relationName) {
		...
		switch ($relationType) {
			...
			case self::BELONGS_TO_MANY:
				$verifyArgs(array('table', 'foreignKey', 'otherKey', 'orderBy'));
			...
			break;
			case self::MORPH_TO:
				$verifyArgs(array('name', 'type', 'id', 'orderBy'));
			...
			case self::MORPH_MANY:
				$verifyArgs(array('type', 'id', 'orderBy'), array('name'));
		...
		if (isset($relation['withTrashed'])) {
			$relationship->withTrashed();
		}

		if (isset($relation['with'])) {
			$relationship->with($relation['with']);
		}
